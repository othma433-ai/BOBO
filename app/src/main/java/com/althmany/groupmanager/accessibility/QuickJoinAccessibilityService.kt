 ...
 */
class QuickJoinAccessibilityService : AccessibilityService(), AccessibilityUiDriver {
    ...
    override fun onCreate() {
        ...
        attachUnifiedExtractorControllers()
        runCatching {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        AccessibilityRuntimeBridge.event(this, eventPackage)
        routeUnifiedExtractorEvent(eventPackage)
    ...
    private fun attachUnifiedExtractorControllers() {
    ...
    private fun routeUnifiedExtractorEvent(packageName: String?) {
    ...
    override fun tapBounds(bounds: Rect?, durationMs: Long): Boolean {
    ...
    private fun unifiedSwipe(fromY: Float, toY: Float, durationMs: Long): Boolean {
        ...
        val gesture = GestureDescription.Builder()
            .addStroke(GestureDescription.StrokeDescription(path, 0, durationMs.coerceIn(72L, 900L)))
            .build()
        return dispatchGesture(gesture, null, null)
    }

    override fun swipeTowardOlderMessages(durationMs: Long): Boolean = unifiedSwipe(0.34f, 0.78f, durationMs)
    override fun swipeTowardNewerMessages(durationMs: Long): Boolean = unifiedSwipe(0.78f, 0.28f, durationMs)
    override fun swipeChatListForward(durationMs: Long): Boolean = unifiedSwipe(0.78f, 0.28f, durationMs)
    override fun swipeChatListBackward(durationMs: Long): Boolean = unifiedSwipe(0.30f, 0.78f, durationMs)
    ...
    private suspend fun completeAndAdvance(
        ...
        val state = try {
            ...
            // A watchdog/parallel observer may have already committed this link. Drop any stale
            // cached pointer immediately and rescan the repository-backed current item instead of
            // repeatedly classifying the old terminal surface.
            invalidateRuntimeCache()
            runtimeDiagnostic(current, "STALE_TERMINAL_HANDOFF", "result already committed or queue advanced; reload current link")
            scanPending.set(true)
            return
        }
