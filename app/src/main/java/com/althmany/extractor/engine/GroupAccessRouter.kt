...
class GroupAccessRouter(private val adapter: WhatsAppUiAdapter) {
    ...
    suspend fun open(
        ...
        // Smart Extraction can prefer exact-name Search because the synchronized registry already
        // knows the next target. The opened chat is still verified before extraction begins.
        if (allowSearchFallback && preferSearch) priority += GroupAccessMethod.SEARCH_FALLBACK
