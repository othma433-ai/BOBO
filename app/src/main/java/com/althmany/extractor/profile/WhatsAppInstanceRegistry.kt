...
object WhatsAppInstanceRegistry {
    ...
    fun available(context: Context, forceRefresh: Boolean = false): List<WhatsAppInstance> {
        ...
        // Best-effort broad discovery lane. Android may restrict visibility, but when an app is
        // visible we accept any package/label that looks like WhatsApp instead of relying on a
        // fixed list of clone package names.
        runCatching {
            @Suppress("DEPRECATION")
            pm.getInstalledApplications(0)
        }.getOrDefault(emptyList()).forEach { ai ->
