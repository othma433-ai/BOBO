        ...
        CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Rtl) {
            ...
            when (screen) {
                ...
                AppScreen.EXTRACT -> V341ExtractionScreen(
                    ...
                    onContinuousScroll = viewModel::setContinuousScrollEnabled,
                    onFastEndVerification = viewModel::setFastEndVerificationEnabled,
                    onFastEndMs = viewModel::setFastEndVerificationMs,
                    onHiddenSearchNavigation = viewModel::setHiddenSearchNavigation,
                    onCheckpointEnabled = viewModel::setCheckpointEnabled,
...
private fun isAccessibilityServiceEnabled(context: Context): Boolean {
    val acceptedClasses = setOf(
        "com.althmany.groupmanager.accessibility.QuickJoinAccessibilityService"
    )
    val enabled = Settings.Secure.getString(context.contentResolver, Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES).orEmpty()
    return enabled.split(':').map { it.trim() }.any { entry ->
        acceptedClasses.any { className -> entry.endsWith("/$className", ignoreCase = true) }
    }
}
