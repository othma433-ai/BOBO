...
object AccessibilityRuntimeBridge {
    @Volatile private var active: AccessibilityUiDriver? = null
    ...
    fun bind(service: AccessibilityUiDriver) {
        active = service
        connected = true
    ...
    fun event(service: AccessibilityUiDriver, packageName: String?) {
    ...
    fun unbind(service: AccessibilityUiDriver) {
        if (active === service) {
            active = null
            connected = false
    ...
    fun current(maxHeartbeatAgeMs: Long = 5_000L): AccessibilityUiDriver? {
    ...
    fun currentEvenIfQuiet(): AccessibilityUiDriver? {
        val service = active ?: return null
